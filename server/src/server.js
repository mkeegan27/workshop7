      ...
      db.collection('feedItems').updateOne({ _id: feedItemId},
        {
          ...
          db.collection('feedItems').findOne( {_id: feedItemId }, function(err,feedItem) {
            ...
            var usList = [feedItem.comments[commentIdx].author];
            usList.concat(feedItem.comments[commentIdx].likeCounter);
            resolveUserObjects(usList, function(err, userMap) {
              if(err){
                return sendDatabaseError(res, err);
              }
              // feedItem.comments[commentIdx].likeCounter.map((userId)=>userMap[userId]);
              // feedItem.comments[commentIdx].author = userMap[feedItem.comments[commentIdx].author].fullName;
              // feedItem.comments.forEach((comment) => {
              //   comment.author = userMap[comment.author];
              //   comment.likeCounter.map((userId)=>userMap[userId]);
              // });
              var comment = feedItem.comments[commentIdx];
              comment.likeCounter.map((userId)=>userMap[userId]);
              comment.author = userMap[comment.author];
